...
def create(term, memo = {}):
    ...
    # elif isinstance(term, ListPL):  # It must already have a tail.
    #     term.head = [create(item, memo) for item in term.head]
    #     term.tail = create(term.tail.name)
    #     memo[str(term)] = term
    elif isinstance(term, list):
        # If the list is empty, it is a Const; otherwise it is a ListPL.
        if term == []:
            memo[str(term)] = Const([])
        elif isinstance(term[-1], Tail):      #???
            head = [create(item, memo) for item in term[:-1]]
            tail = create(term[-1].name, memo)
            memo[str(term)] = ListPL(head, tail)
        else: 
            memo[str(term)] = ListPL([create(item, memo) for item in term])
...
class ListPL(Term):
    ...
    def __init__(self, head, tail = None):
        ...
        self.tail = tail
        self.children = []
    # Returns a tuple of (listWithShorterHead, listWithLongerHead).
    @staticmethod
    def shortLong(lst1, lst2):
        if len(lst1) < len(lst2):
            return (lst1, lst2)
        return (lst2, lst1)
    def __eq__(self, other):
        if not isinstance(other, ListPL):
            return super().__eq__(other)
        if len(self) != len(other) and (not self.tail or not other.tail):
            return False
        short, long = self.shortLong(self, other)
        if short.head == long.head[:len(short.head)]:
            return True
        return False
    # def unifyWith(self, altArg):
    ...
    def value(self):
        if not self.tail:
            return self.head
        if self.tail.value != "Undefined":
            self.head.extend(self.tail.value)
            self.tail = None
            return self.head
...
# Connect a tail to a list by doing [] | Tail(T). Tail is actually just a Var.
# Tail = Var

X = Tail(3)
Y = Var(3)
print(isinstance(X, Tail))